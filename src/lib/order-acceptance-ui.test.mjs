...
describe("order acceptance UI wiring", () => {
  test("cart page disables checkout when the acceptance window is closed", () => {
    ...
    expect(source).toContain('resolveOrderedByFullName("", session.user)');
    expect(source).toContain(
      "ФИО берется из настроек профиля и больше не заполняется при оформлении заказа.",
    );
    expect(source).toContain("Укажите ФИО оформляющего в настройках профиля.");
    expect(source).not.toContain("updateDraft({ orderedByFullName");
    expect(source).not.toContain('placeholder="Например, Иван Иванов"');
  });
