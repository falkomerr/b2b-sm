...
describe("cart quantity layout", () => {
  ...
  test("quantity control keeps the kg input shrinkable on narrow screens", () => {
    ...
    expect(source).toContain('className="w-12 min-w-0 border-none bg-transparent text-center');
    expect(source.indexOf("onClick={onDecrement}")).toBeLessThan(source.indexOf("<input"));
    expect(source.indexOf("<input")).toBeLessThan(source.indexOf("onClick={onIncrement}"));
    expect(source).not.toContain(
      'className="flex h-8 min-w-0 items-center gap-1.5 rounded-full border border-[#dfe3eb] bg-white px-2.5 sm:px-3"',
    );
  });
});
