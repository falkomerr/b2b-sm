...
describe("order details page regressions", () => {
  ...
  test("editable quantity controls reuse shared increment and decrement helpers", () => {
    const source = readProjectFile("src/app/orders/[orderId]/page.tsx");

    expect(source).toContain("decrementQuantity(item.quantity, item.unit)");
    expect(source).toContain("incrementQuantity(item.quantity, item.unit)");
    expect(source).not.toContain("item.quantity - 1");
    expect(source).not.toContain("item.quantity + 1");
  });
});
